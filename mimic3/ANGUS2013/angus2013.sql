...
DROP TABLE IF EXISTS ch0_infection_pop; CREATE TABLE IF NOT EXISTS ch0_infection_pop( subject_id integer, hadm_id integer);
...
DROP TABLE IF EXISTS ch1_general_vars_temp; CREATE TABLE IF NOT EXISTS ch1_general_vars_temp( subject_id integer, hadm_id integer);
INSERT INTO ch1_general_vars_temp (subject_id, hadm_id)(
...
DROP TABLE IF EXISTS ch1_general_vars_hr;CREATE TABLE IF NOT EXISTS ch1_general_vars_hr( subject_id integer, hadm_id integer);
INSERT INTO ch1_general_vars_hr (subject_id, hadm_id)(
	SELECT  DISTINCT   ce.subject_id, ce.hadm_id
    FROM mimiciii.chartevents ce
    WHERE
		( ce.itemid IN (220045, 220048, 211) AND ce.valuenum > 90) --  heart rate 
);
DROP TABLE IF EXISTS ch1_general_vars_rr; CREATE TABLE IF NOT EXISTS ch1_general_vars_rr( subject_id integer, hadm_id integer);
INSERT INTO ch1_general_vars_rr (subject_id, hadm_id)(
	SELECT  DISTINCT   ce.subject_id, ce.hadm_id
    FROM mimiciii.chartevents ce
    WHERE
		(ce.itemid IN (618, 614, 615, 618, 651, 1151, 1884, 3603, 3337) AND ce.valuenum > 20) -- respi rate
);
DROP TABLE IF EXISTS ch1_general_vars_glasgow; CREATE TABLE IF NOT EXISTS ch1_general_vars_glasgow( subject_id integer, hadm_id integer);
INSERT INTO ch1_general_vars_glasgow (subject_id, hadm_id)(
	SELECT  DISTINCT   ce.subject_id, ce.hadm_id
    FROM mimiciii.chartevents ce
    WHERE
		( ce.itemid IN (198) AND ce.valuenum < 12 ) -- glasgow
);
DROP TABLE IF EXISTS ch1_general_vars_gly; CREATE TABLE IF NOT EXISTS ch1_general_vars_gly( subject_id integer, hadm_id integer);
INSERT INTO ch1_general_vars_gly (subject_id, hadm_id)(
	SELECT  DISTINCT   ce.subject_id, ce.hadm_id
    FROM mimiciii.chartevents ce
    WHERE
...
DROP TABLE IF EXISTS ch1_general_vars_fluid; CREATE TABLE IF NOT EXISTS ch1_general_vars_fluid( subject_id integer, hadm_id integer);
INSERT INTO ch1_general_vars_fluid (subject_id, hadm_id)(
...
DROP TABLE IF EXISTS ch2_infla_vars_wb; CREATE TABLE IF NOT EXISTS ch2_infla_vars_wb( subject_id integer, hadm_id integer);
INSERT INTO ch2_infla_vars_wb (subject_id, hadm_id)(
	SELECT  DISTINCT   ce.subject_id, ce.hadm_id
	FROM
...
DROP TABLE IF EXISTS ch2_infla_vars_crp; CREATE TABLE IF NOT EXISTS ch2_infla_vars_crp( subject_id integer, hadm_id integer);
INSERT INTO ch2_infla_vars_crp (subject_id, hadm_id)(
	SELECT  DISTINCT   ce.subject_id, ce.hadm_id
	FROM
	mimiciii.chartevents ce
	WHERE  
...
-- CHAP 3 HEMO_VARS
DROP TABLE IF EXISTS ch3_hemo_vars_hypo_art; CREATE TABLE IF NOT EXISTS ch3_hemo_vars_hypo_art( subject_id integer, hadm_id integer);
INSERT INTO ch3_hemo_vars_hypo_art (subject_id, hadm_id)(
	SELECT DISTINCT ce.subject_id, ce.hadm_id
	FROM 
	mimiciii.chartevents ce
...
DROP TABLE IF EXISTS ch3_hemo_vars_mix_ven; CREATE TABLE IF NOT EXISTS ch3_hemo_vars_mix_ven( subject_id integer, hadm_id integer);
INSERT INTO ch3_hemo_vars_mix_ven (subject_id, hadm_id)(
	SELECT DISTINCT ce.subject_id, ce.hadm_id
	FROM 
	mimiciii.chartevents ce
    WHERE valuenum IS NOT NULL AND (
...
DROP TABLE IF EXISTS ch3_hemo_vars_card_index; CREATE TABLE IF NOT EXISTS ch3_hemo_vars_card_index( subject_id integer, hadm_id integer, hypo_art boolean, syst_press boolean, mean_art_press boolean, ven_o2_sat boolean, card_index boolean);
INSERT INTO ch3_hemo_vars_card_index (subject_id, hadm_id)(
	SELECT DISTINCT ce.subject_id, ce.hadm_id
	FROM 
	mimiciii.chartevents ce
    WHERE valuenum IS NOT NULL AND (
...
DROP TABLE IF EXISTS ch4_organ_dysf_art_hypox; CREATE TABLE IF NOT EXISTS ch4_organ_dysf_art_hypox ( subject_id integer, hadm_id integer);
INSERT INTO ch4_organ_dysf_art_hypox (subject_id, hadm_id) (
...
DROP TABLE IF EXISTS ch4_organ_dysf_creat; CREATE TABLE IF NOT EXISTS ch4_organ_dysf_creat( subject_id integer, hadm_id integer);
-- TODO: variable window on diffetime ch4_organ_dysf_creat -> test
INSERT INTO ch4_organ_dysf_creat (subject_id, hadm_id) (
...
DROP TABLE IF EXISTS ch4_organ_dysf_thromb; CREATE TABLE IF NOT EXISTS ch4_organ_dysf_thromb( subject_id integer, hadm_id integer);
INSERT INTO ch4_organ_dysf_thromb (subject_id, hadm_id) (
...
DROP TABLE IF EXISTS ch4_organ_dysf_billi; CREATE TABLE IF NOT EXISTS ch4_organ_dysf_billi( subject_id integer, hadm_id integer);
INSERT INTO ch4_organ_dysf_billi (subject_id, hadm_id) (
	SELECT ce.subject_id, ce.hadm_id
	FROM 
	mimiciii.chartevents ce
    WHERE
...
DROP TABLE IF EXISTS ch4_organ_dysf_plat; CREATE TABLE IF NOT EXISTS ch4_organ_dysf_plat( subject_id integer, hadm_id integer);
INSERT INTO ch4_organ_dysf_plat (subject_id, hadm_id) (
	SELECT ce.subject_id, ce.hadm_id
	FROM 
	mimiciii.chartevents ce
    WHERE
...
DROP TABLE IF EXISTS ch4_organ_dysf_urin; CREATE TABLE IF NOT EXISTS ch4_organ_dysf_urin(subject_id integer, hadm_id integer);
INSERT INTO ch4_organ_dysf_urin (subject_id, hadm_id) (
...
FROM mimiciii.outputevents WHERE  itemid IN (40055, 40069, 40094, 40715, 226627)  ),
              tmp as ( SELECT o1.subject_id, o1.hadm_id,  o1.position, o2.position, o1.charttime, o2.charttime, EXTRACT(epoch FROM (o2.charttime - o1.charttime))/3600 as duree, coalesce(o2.value,o1.value) as val,   
               ...
               SELECT subject_id, hadm_id FROM tmp WHERE result IS TRUE
);
...
INSERT INTO ch5_tissue_perf (subject_id, hadm_id) (
	SELECT  DISTINCT ce.subject_id, ce.hadm_id
	FROM mimiciii.chartevents ce
    WHERE
